...
if (isset($_POST['search'])) {
  ...
  $searchQuery = "SELECT * FROM `posts` WHERE `post` LIKE '%$search%' OR `title` LIKE '%$search%' OR `category` LIKE '%$search%'";
...
<header  class="navbar navbar-dark sticky-top bg-dark flex-md-nowrap p-0 shadow">
  ...
  <form action="adminposts.php" method="post"  role="form" class="form-control form-control-dark w-100">
    ...
    <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
      <div class="position-sticky pt-3">
        <ul class="nav flex-column">
          ...
          <li class="nav-item">
            <a class="nav-link active" href="adminposts.php">
      ...
      new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{
            ...
            data: [<?php
            $posts = [];
            $postUser = "SELECT `users`.`username`,COUNT(post) FROM `posts` INNER JOIN `users` ON `users`.`user_id`=`posts`.`user_id` GROUP by `users`.`username` ORDER BY `users`.`user_id`";
      ...
      <div class="table-responsive">
        <table id="myTable" class="display">
          ...
          <form action="edit.php" method="GET">
            <?php
            $query = "SELECT * FROM `posts` INNER JOIN `users` ON `users`.`user_id`=`posts`.`user_id`";
            $usersResult = mysqli_query($connection,$query) or die("bad conn");
                ...
                while ($row = mysqli_fetch_array($usersResult)) {
                    ...
                    echo " 
                    <tr>
                    <td><button onclick='location.href=`editposts.php?post_id={$id}`' class='btn btn-primary'></button></td>
                    <td>".$id."</td>
                    <td>".$row['username']."</td>
                    <td>".$row['title'] ."</td>
                    <td>" . $row['post']."</td>
