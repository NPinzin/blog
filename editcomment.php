...
if (isset($_POST['submitButton'])) {
    ...
    $updateQuery = "UPDATE `comments` SET `comment`= '$content' WHERE `comment_id`=$id";
...
if (isset($_POST['deleteComment'])) {
  $id = $_GET['comment_id'];
  $content = mysqli_real_escape_string($connection,$_POST['content']);
  $updateQuery = "UPDATE `comments` SET `comment` = '$content', `public` = 0 WHERE `comment_id` = $id";
...
<form action="editcomment.php?comment_id=<?php echo $id?>" method="POST">
    <?php
    // selecting posts
      $user = $_SESSION['user_id'];
      $post_id = $_GET['comment_id'];
      $comQ = "SELECT `comments`.`comment`,`posts`.`title`,`users`.`firstName`, `users`.`lastName`, `comments`.`public`,`comments`.`timestamp` FROM `comments` INNER JOIN `posts` ON `posts`.`post_id`=`comments`.`post_id` INNER JOIN `users` ON `users`.`user_id`=`comments`.`user_id` WHERE `comment_id` = $id";
      $comRes = mysqli_query($connection,$comQ);
    ...
    <input name='submitButton'type="submit" value='Edit Comment!' class="btn btn-primary"></input>
    <input name='deleteComment'type="submit" value='Delete Comment' class="btn btn-danger"></input>
        </form>
    </body>
</html>