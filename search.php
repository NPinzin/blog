  ...
  <div class="row g-5">
    <div class="col-md-8">
      <h3 class="pb-4 mb-4 fst-italic border-bottom">
        ...
        // selecting posts
          $search = $_GET['search'];
          $postQuery = "SELECT * FROM `posts` INNER JOIN `users` ON `users`.`user_id`=`posts`.`user_id` INNER JOIN `categories` ON `posts`.`category_id`=`categories`.`category_id` WHERE `posts`.`post` LIKE '%$search%' OR `categories`.`category` LIKE '%$search%' OR `users`.`firstName` LIKE '%$search%' OR `users`.`lastName` LIKE '%$search%' OR `users`.`username` LIKE '%$search%' OR `posts`.`title` LIKE '%$search%' OR `posts`.`timestamp` LIKE '%$search%' AND `public` = 1 ORDER BY `post_id` DESC";
